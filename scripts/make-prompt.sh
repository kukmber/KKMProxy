...
cat <<'EOF'
КАК ОТВЕТИТЬ
1. Сначала коротким списком: что переносим и что пропускаем, по одному
   предложению на правку.
2. Потом — ПОЛНОЕ содержимое каждого изменённого файла в отдельном блоке
   кода, готовое к сохранению. Имя файла укажи строкой перед блоком.
   Файлы, которые не меняются, присылать не нужно.
3. В конце — что проверить в приложении после установки.

Полные файлы нужны потому, что я сохраняю их как есть и собираю
приложение автоматически. Если ответ получается слишком длинным —
меняй за раз только один файл.
EOF
} > "$OUT"
